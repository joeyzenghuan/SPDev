...
# site admin /_layouts/15/mngsiteadmin.aspx
$Site = $Ctx.Site
...
$LibraryPath = "sites/modernTeamSite2/Shared Documents"
...
ForEach ($RecycleBinItem in $RecycleBinItems) {
    ...
    if ($RecycleBinItem.DirName.Contains($LibraryPath)) {
        Write-host -f Yellow $RecycleBinItem.LeafName
        # Write-host -f Yellow $RecycleBinItem.DirName
